...
namespace HireMatch.WebAPI.Controllers
{
    public class SkillsController : BaseCRUDController<SkillResponse, SkillSearchObject, SkillInsertRequest, SkillUpdateRequest>
    {
        ...
        public override async Task<IActionResult> Post([FromBody] SkillInsertRequest request)
            => await base.Post(request);

        [HttpPut("{id}")]
        [Authorize(Roles = "Admin")]
        public override async Task<IActionResult> Put(int id, [FromBody] SkillUpdateRequest request)
